            ...
            ZStack {
                ...
                VStack {
                    ...
                    List {
                        ...
                        //ForEach(pChecks, id: \.self) { pCheck in
                        //pChecks.forEach {
                            //HStack { Text(self.pSymptoms[0]); Spacer(); $0 }  /// most likely place to copy pChecks and not use the actual thing; thing about structs copying in SwiftUI
    ...
    func submitButton() { /// func and var names are lowercase
        ...
        for i in 0..<pChecks.count {
            ...
            if pChecks[i].checkState == true {
                let pSymptom = PSymptoms(context: self.managedObjectContext)
                pSymptom.pSymptomName = pSymptoms[i]
                pSymptom.pCreatedAt = Date()
                pSymptom.pCheckState = pChecks[i].checkState
                print("---- added ----")
            }
        }
        //save()
